"""
Financial Module for DTC with TES
Pro forma financial model with LCOE calculations including Thermal Energy Storage
Extends base DTC_CPLEX financials with TES CapEx/OpEx, depreciation, and ITC
"""
import numpy as np
import pandas as pd
...
def build_pfx(vars, svar, opsx, dftotin, basepath, mode):
    ...
    # TES capex (thermal storage + steam turbine)
    if svar.get('tes_kWh', 0)==0 or vars.get('structure_tes', 'Integrated')=='Tolled':
        tes_capex = 0
    else:
        # Thermal storage cost: $40/kWh thermal (Rondo/Anora validated)
        # Steam turbine cost: ~$1000/kW electric output capacity
        tes_storage_capex = vars.get('capex_TES_storage', 40) * svar['tes_kWh'] / 1000.0  # $/MWh → $M
        tes_turbine_capex = vars.get('capex_TES_turbine', 1000) * svar['tesD_kW'] * vars['tes_st_eff'] / 100.0 / 1000.0  # $/MW elec
        tes_capex = tes_storage_capex + tes_turbine_capex
    pfx.at['capex_TES', str(0)] = -tes_capex
    ...
    # TES opex (very low O&M, thermal storage is passive)
    if svar.get('tes_kWh', 0)==0:
        pass
    elif vars.get('structure_tes', 'Integrated')=='Tolled':
        # Tolled structure - capacity charge per month
        for y in np.arange(1, vars['proj_life']+1, 1):
            escfctr = (1+vars['fin_esc']) ** (y-1)
            tes_toll_mw = svar['tesD_kW'] * vars['tes_st_eff'] / 100.0 / 1000.0  # MW electric capacity
            pfx.at['toll_TES', str(y)] = -escfctr * (tes_toll_mw * vars.get('toll_TES', 5000) * 12.0)
    else:
        # Integrated structure - O&M cost (typically $2-5/kWh/yr for thermal storage)
        for y in np.arange(1, min(vars.get('life_TES', 30), vars['proj_life'])+1, 1):
            escfctr = (1+vars['fin_esc']) ** (y-1)
            pfx.at['o&m_TES', str(y)] = escfctr * (-1*vars.get('opex_TESfix', 3) * svar['tes_kWh']/1000.0)
    ...
    for y in np.arange(1, vars['proj_life']+1,1):
        ...
        if y<= vars['y_deprec']:
            ...
            pfx.at['dep_TES', str(y)] = pfx.at['capex_TES', str(0)] * (1-vars.get('fin_TESitc', 0)/2) / vars['y_deprec']
    ...
    # TES itc (eligible as energy storage under IRA)
    if vars.get('structure_tes', 'Integrated')=='Integrated':
        pfx.at['itc_TES', str(1)] = - pfx.at['capex_TES', str(0)] * (vars.get('fin_TESitc', 0.30)) * vars['fin_tccapture']
